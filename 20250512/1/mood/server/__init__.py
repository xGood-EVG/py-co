...
def translate(msg, locale, fmt, n=0):
    """Performs translation for each phrase using gettext"""
    if n:
        for x in msg.split("\n"):
...
def start_server(port=1337):
    """Starts server on given port, default 1337"""
    ...
    LOCALES = {
        "ru_RU": translation("mood_lang", os.path.join(os.path.dirname(__file__), "../../po"), languages=["ru_RU.UTF-8"], fallback=False),
