...
with open(os.path.join(os.path.dirname(__file__), "..", "common", "bat.txt")) as f:
...
class cmd_line(cmd.Cmd):
    ...
    def do_documentation(self, args):
        """Opens a browser window with docs"""
        webbrowser.open(os.path.join(os.path.dirname(__file__), "../../build/html/index.html"))
    ...
    def postcmd(self, stop: bool, line: str) -> bool:
        """Sets timeout after each command in case of file input"""
        self.timeout()
        return super().postcmd(stop, line)

    def postloop(self):
        """Normal exit from command line loop"""
        return 1
...
def start_client(login="login", file_=""):
    """Starts client application with provided login and input file, defaults are login and STDIN"""
